...
If you use Windows or MacOS, please see additional instructions toward
the bottom.
...
PKG.mac and PKG.lisp. In the top-level template directory you will
find a doc/ directory that will contain your PKG.info doc file and two
...
command, then both commands MUST be in the same '@group'. Also,
a single example command cannot span multiple lines, it must
...
$ ./create_docs.sh -M /path/to/maxima/source-code

This should be the only option you need to set manually. The other
options are determined automatically. When run, this should create
PKG.info, PKG.pdf, a PKG_html/ directory containing html-related doc
